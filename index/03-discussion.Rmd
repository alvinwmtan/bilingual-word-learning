...
# Discussion
In our study, we addressed the baseline problem by using an item-level modelling approach, allowing us to have a better understanding of how TEs affect early word learning in bilingual children, and how this interacts with other child- and item-level predictors.
Revisiting our hypotheses, we found that:

1. Greater age, frequency, and language exposure (i.e., more language input) predicted greater word knowledge;
2. Greater concreteness and lower MLU-w predicted greater word knowledge;
3. Knowing a word predicted knowing its corresponding TE, particularly for younger children;
4. The role of TE knowledge was moderated by the phonological similarity between the TEs;
5. There was high consistency of predictor importance across datasets and language combinations; and
6. The contribution of some predictors varied across lexical categories.
...
The results also suggest that other factors affecting word learning in the monolingual context [see @braginskyConsistencyVariabilityChildren2019] also have a similar effect in the bilingual context, including concreteness and MLU-w, providing support for Hypotheses 2. 
...
These findings are reinforced by the result that the effects of such variability are relatively consistent across datasets, suggesting that at a high level, the factors affecting lexical acquisition are similar across these populations.
...
Notably, this set of languages reflects the distribution of languages studied in the literature, likely because the United States and Canada are the countries in which much of this research is being conducted.
...
Drawing from the standard model, we adopted a modelling approach that was able to address the baseline problem, and that could incorporate both child- and item-level variability. 
...
Our main goal was to understand the role of TEs, and we found that TE knowledge was indeed a useful source of language information for bilingual children, and that this effect was accentuated for younger children and when the TEs have high phonological similarity.
Furthermore, the role of TEs was consistent across populations, indicating a robust effect, but varied across lexical categories, suggesting variability in how words of different lexical categories are learnt.
