...
datasets_small |> 
  ...
  rename(`$N_P$ (boys)` = Ppts,
         `$N_A$` = Admins) |> 
  knitr::kable(caption = "Descriptive statistics of datasets used, after excluding participants with zero exposure to either language. $N_P$: number of participants; $N_A$: number of administrations; Prop English: proportion of exposure to English.", 
               booktabs = T,
               escape = F) |> 
...
We included all children and administrations that were marked as 'bilingual' by the original authors, but excluded administrations from children who had zero exposure to one of their languages ($N_A$ = `r ((datasets_small |> pull(admins_orig) |> sum()) - (datasets_small |> pull(Admins) |> sum())) |> apa_num(format="d")`), resulting in a final sample of `r datasets_small |> pull(Admins) |> sum() |> apa_num(format="d")` administrations from `r datasets_small |> pull(Ppts) |> sum() |> apa_num(format="d")` children.[^02-01]
The details of the datasets are shown in Table \@ref(tab:datasets-small). 

[^02-01]: The datasets have four details of note.
...
Third, some children had low language exposures in one of their languages (<10%). 
...
Nonetheless, we were interested in children who had at least some exposure to two languages, and therefore excluded those who were only exposed to one language.
Fourth, not all datasets included race and ethnicity information, so these have been omitted.
...
[^02-04]: Note that including random intercepts for child helped to account for the fact that some children contributed multiple administrations at different ages.
...
[^02-05]: Estimates are means of maximum a posteriori probability estimates. Credible intervals were derived by taking the mean of the extents of the highest density intervals, and are approximate.
...
First, there was a main effect of TE knowledge ($\bar\beta =$ `r mp_t |> filter(Parameter == "b_te_known") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_t |> filter(Parameter == "b_te_known") |> pull(CI_low) |> mean()`, `r mp_t |> filter(Parameter == "b_te_known") |> pull(CI_high) |> mean()`]), although there was relatively large variability, with the posterior distribution of one model having an 89\% CI that was >\ 95% within the region of practical equivalence.
There was also a negative TE interaction effect with age ($\bar\beta =$ `r mp_t |> filter(Parameter == "b_age_t:te_known") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_t |> filter(Parameter == "b_age_t:te_known") |> pull(CI_low) |> mean()`, `r mp_t |> filter(Parameter == "b_age_t:te_known") |> pull(CI_high) |> mean()`]) and with frequency ($\bar\beta =$ `r mp_t |> filter(Parameter == "b_freq_t:te_known") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_t |> filter(Parameter == "b_freq_t:te_known") |> pull(CI_low) |> mean()`, `r mp_t |> filter(Parameter == "b_freq_t:te_known") |> pull(CI_high) |> mean()`]); in other words, TE knowledge provided a greater boost to younger children and for less frequent words. 
Additionally, there was a positive TE interaction effect with proportion of exposure to the language ($\bar\beta =$ `r mp_t |> filter(Parameter == "b_lang_prop_t:te_known") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_t |> filter(Parameter == "b_lang_prop_t:te_known") |> pull(CI_low) |> mean()`, `r mp_t |> filter(Parameter == "b_lang_prop_t:te_known") |> pull(CI_high) |> mean()`]); it is not clear how this effect should be interpreted, although three of the posterior distributions had 89\% CIs that were 14--82% within the region of practical equivalence, suggesting that it is undecided whether to accept or reject the null hypothesis in this case. 
...
There was also an additional interaction effect between TE knowledge and phonological similarity  ($\bar\beta =$ `r mp_f |> filter(Parameter == "b_te_known:overlap_t") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_f |> filter(Parameter == "b_te_known:overlap_t") |> pull(CI_low) |> mean()`, `r mp_f |> filter(Parameter == "b_te_known:overlap_t") |> pull(CI_high) |> mean()`]), such that TE knowledge provided a greater boost when the TEs have greater phonological similarity.
All other interaction effects had posterior distributions that largely fell within the region of practical equivalence.
...
First, the age-by-TE interaction was stronger for verbs ($\bar\beta =$ `r mp_lc_f |> filter(Parameter == "b_age_t:te_known", lexical_class == "verbs") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_lc_f |> filter(Parameter == "b_age_t:te_known", lexical_class == "verbs") |> pull(CI_low) |> mean()`, `r mp_lc_f |> filter(Parameter == "b_age_t:te_known", lexical_class == "verbs") |> pull(CI_high) |> mean()`]) than for other lexical categories, suggesting a greater TE boost for verbs in younger children.
Second, the frequency-by-TE interaction was positive for adjectives ($\bar\beta =$ `r mp_lc_f |> filter(Parameter == "b_freq_t:te_known", lexical_class == "adjectives") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_lc_f |> filter(Parameter == "b_freq_t:te_known", lexical_class == "adjectives") |> pull(CI_low) |> mean()`, `r mp_lc_f |> filter(Parameter == "b_freq_t:te_known", lexical_class == "adjectives") |> pull(CI_high) |> mean()`]) whereas it was negative for other lexical categories; this may have been driven by particular semantic categories (e.g., colour words), which both have higher frequency and are commonly learnt by children in both languages.
Third, the phonological similarity-by-TE interaction was positive for nouns ($\bar\beta =$ `r mp_lc_f |> filter(Parameter == "b_te_known:overlap_t", lexical_class == "nouns") |> pull(Estimate) |> mean()`, 89\% CI [`r mp_lc_f |> filter(Parameter == "b_te_known:overlap_t", lexical_class == "nouns") |> pull(CI_low) |> mean()`, `r mp_lc_f |> filter(Parameter == "b_te_known:overlap_t", lexical_class == "nouns") |> pull(CI_high) |> mean()`]) whereas it was equivalent to zero for other lexical categories, suggesting that phonological similarity between TE pairs was particularly important for nouns, supporting the findings from @schelletterBilingualChildrenLexical2005.
