# Modelling word production {#word-prod}
In our study, we modelled bilingual children's productive vocabulary from CDI datasets, aiming to understand the standard model as a model of word learning in bilingual contexts, and to investigate the contribution of TEs to word learning.
We first constructed a base model by adapting the standard model (which was initially developed on monolinguals) for the bilingual context.
...
Next, we added TEs and phonological similarity as additional predictive variables to determine if they explained some of the variance in bilingual word knowledge. 
We then examined the contributions of these variables, with the following hypotheses regarding their magnitudes:

1. Older children have better word knowledge, and greater frequency and language exposure predict better word knowledge, as does the interaction between them [as predicted by the standard model; see @kachergisStandardModelEarly2022];
...
We also conducted additional secondary analyses to consider how these effects would hold across populations and across lexical categories, with the following hypotheses:
...
6. The importance of different predictors may vary among different lexical categories [as different sources of linguistic information may vary in importance across lexical categories, e.g., @goodmanDoesFrequencyCount2008; @braginskyConsistencyVariabilityChildren2019].
...
datasets_small |> 
  rename(`$N_P$` = Ppts,
         `$N_A$` = Admins) |> 
  knitr::kable(caption = "Descriptive statistics of datasets used. $N_P$: number of participants; $N_A$: number of administrations.", 
               booktabs = T,
               escape = F) |> 
...
[^04-02]: We considered a few possible ways to specify the interaction term between exposure and frequency.
...
## Results
We plot example word learning trajectories for "dog" and "jump" from one dataset in Figure \@ref(fig:mr-demo), collapsed across children binned by language exposure.

(ref:mr-demo-cap) Example trajectories for "dog" and "jump" derived from Model 1 fitted to the English--Spanish dataset from Marchman. Panel columns reflect proportion of language exposure in the corresponding language (i.e., exposure proportion of English for the first row, and exposure proportion of Spanish for the second row).
...
(ref:mr-pred-cap) Effect of word frequency on word production, derived from Model 1 fitted on the English--Spanish dataset from Marchman. Panel columns reflect proportion of language exposure in the corresponding language (i.e., exposure proportion of English for the first row, and exposure proportion of Spanish for the second row).
...
[^04-03]: Estimates are means of maximum a posteriori probability estimates. Credible intervals were derived by taking the mean of the extents of the highest density intervals, and are approximate.
...
The main effects were largely similar to those of Model 1, so we focus on the effects related to TEs.
...
As such, we subdivided each dataset into lexical categories (nouns, verbs, adjectives, function words, and others) and reran Model 3 on each subset,[^04-04] dropping the "others" category due to its internal heterogeneity.

[^04-04]: This approach had better interpretability relative to the alternative, which would be to include lexical category as a moderator variable, thereby greatly increasing the number of interaction terms.
