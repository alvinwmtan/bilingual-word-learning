...
Finally, we explore whether this tendency is more likely for TEs that sound more similar in the child's two languages than words that have more distinct phonological forms.
...
In this case, the expected proportion of a bilingual child’s vocabulary composed of TE pairs is $\mathbb{E}(\textrm{\%TE}) = 2 P_1 P_2 \,/\, (P_1 + P_2)$.[^01-02]
...
These results suggest that phonological similarity may be an important factor to consider in understanding the role of TEs in bilingual word learning.
...
1. Older children have better word knowledge, and greater frequency and language exposure predict better word knowledge, as does the interaction between them [as predicted by the standard model; see @kachergisStandardModelEarly2022]; and
2. Higher concreteness and lower MLU-w predict better word knowledge [from previous age of acquisition modelling work, e.g., @braginskyConsistencyVariabilityChildren2019].
...
4. Greater phonological similarity predicts better word knowledge only when the word's TE is known [based on studies suggesting that phonologically similar TEs are learnt earlier, e.g., @mitchellCognatesAreAdvantaged2022; @umbelMeasuringBilingualChildren1992].
