...
In other words, we investigate whether or not children can use their knowledge in one language to bootstrap language learning in another.
...
In other words, just knowing more words means that by chance, more of them happen to be translation equivalents.
...
They found that bilingual children produce a higher proportion of TE pairs than would be expected from simulated means.
...
They estimated the potentially learnable vocabulary at any given age as the 90th percentile monolingual norm for the language (rather than having it constant at the full size of the corresponding CDI).
Thus, the probability of knowing any word out of the potentially learnable vocabulary varies with age, accounting for the distribution of ages of acquisition across CDI items.
Using this model, they found that children with smaller vocabularies (who tend to be younger) have a preference towards acquiring TEs, whereas children with larger vocabularies (who tend to be older) are neutral towards acquiring TEs, in comparison with expected baseline values. 
