...
get_uni_lemmas <- function(language) {
  item_data <- get_item_data(language, "WS")
  uni_lemmas <- item_data |>
    filter(!is.na(uni_lemma)) |>
    select(language, uni_lemma, item_definition, lexical_category, category) |>
    distinct() |>
...
get_token_metrics <- function(lang, metric_funs = default_metric_funs,
  ...
  message(glue("Getting token metrics for {lang}"))
...
get_uni_lemma_metrics <- function(lang, uni_lemma_map, import_data = NULL) {
  ...
  message(glue("Getting unilemma metrics for {lang}"))
  ...
  metrics_mapped <- tokens_mapped |>
    ...
    select(uni_lemma, tokens = token,
           where(\(x) is_integer(x) || is_double(x))) |>
    group_by(uni_lemma) |>
    distinct() |>
    filter(!is.na(count))
